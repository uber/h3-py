"""Bindings for the functions available in libh3"""
...
LinkedGeoCoord._fields_ = [
    ('vertex', GeoCoord),
    ('next', POINTER(LinkedGeoCoord))
]
...
LinkedGeoLoop._fields_ = [
    ('first', POINTER(LinkedGeoCoord)),
    ('last', POINTER(LinkedGeoCoord)),
...
def hexagon_c_array_to_set(h3_addresses):
    return {
        ...
        for hexagon_address in h3_addresses
        if hexagon_address != 0
...
def h3_to_geo_cell_bndry(h3_address):
    """Deprecated."""
    return h3_to_geo_boundary(h3_address)
...
def hex_ring(h3_address, ring_size):
    ...
    if success != 0:
        ...
        return k_ring(h3_address, ring_size) - \
...
def hex_ranges(h3_address_list, ring_size):
    ...
    for i in range(0, num_hexagons):
        ...
        for j in range(0, array_len / num_hexagons):
            # Mostly from
            ...
            # divided by 6 ceiled.
            ring_index = int(math.floor(
                (1 + math.sqrt(1 + 8 * math.ceil(j / 6.0))) /
                2
            )) - 1
