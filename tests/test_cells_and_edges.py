...
def test_get_num_cells():
    ...
    out = {k: h3.get_num_cells(k) for k in expected}
...
def test_average_hexagon_area():
    ...
    out = {k: h3.average_hexagon_area(k, unit='km^2') for k in expected_in_km2}
...
def test_average_hexagon_edge_length():
    ...
    out = {
        res: h3.average_hexagon_edge_length(res, unit='km') for res in expected_in_km
    }
...
def test_origin_to_directed_edges():
    ...
    destinations = {h3.get_directed_edge_destination(e) for e in edges}
...
def test_get_res0_cells():
    ...
    # resolution
    assert all(map(lambda h: h3.get_resolution(h) == 0, out))
...
def test_from_local_ij_error():
    ...
    out = {h3.local_ij_to_cell(h, i, j) for i, j in goodies}
