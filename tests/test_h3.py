...
class TestH3Core(unittest.TestCase):
    ...
    def test_h3_to_geo(self):
        ...
        self.assertAlmostEqual(latlng[0], 37.34579337536848, None,
                               'lat center is ok')
        self.assertAlmostEqual(latlng[1], -121.97637597255124, None,
                               'lng center is ok')
    ...
    def test_h3_to_geo_boundary(self):
        ...
        expectedlatlngs = [[37.271355866731895, -121.91508032705622], [
            ...
            -121.9235499963016], [37.42012867767778, -122.0377349642703],
    ...
    def test_h3_to_geo_boundary_geo_json(self):
        ...
        for i in range(len(lnglats)):
            self.assertAlmostEqual(lnglats[i][0], expectedlnglats[i][0], None,
                                   'lng is ok')
            self.assertAlmostEqual(lnglats[i][1], expectedlnglats[i][1], None,
                                   'lat is ok')
    ...
    def test_many_hex_ranges(self):
        ...
        hex_ranges_even_larger = h3.hex_ranges(list(h3.k_ring('8928308280fffff', 5)), 5)
        self.assertEqual(91, len(list(hex_ranges_even_larger.keys())))

        hexagons_larger = hex_ranges_even_larger['8928308280fffff']

        self.assertEqual(6, len(hexagons))
        self.assertEqual(1, len(hexagons[0]))
        self.assertEqual(6, len(hexagons[1]))
        self.assertEqual(12, len(hexagons[2]))
        self.assertEqual(18, len(hexagons[3]))
        self.assertEqual(24, len(hexagons[4]))
        self.assertEqual(30, len(hexagons[5]))
    ...
    def test_get_h3_unidirectional_edge(self):
        self.assertTrue(
            isinstance(
                h3.get_h3_unidirectional_edge('8928308280fffff',
                                              '8928308280bffff'), str))
    ...
    def test_get_origin_h3_index_from_unidirectional_edge(self):
        self.assertEqual(
            '8928308280fffff',
            h3.get_origin_h3_index_from_unidirectional_edge(
                '11928308280fffff'))
    ...
    def test_h3_distance(self):
        self.assertEqual(0, h3.h3_distance('89283082993ffff',
                                           '89283082993ffff'))
        self.assertEqual(1, h3.h3_distance('89283082993ffff',
                                           '8928308299bffff'))
        self.assertEqual(5, h3.h3_distance('89283082993ffff',
                                           '89283082827ffff'))
